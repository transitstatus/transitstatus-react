...
export const activateSelectorPopup = (e, features, map, agencyData, singleRouteID) => {
  ...
  const predictions = finalItems.map((item, i) => {
    console.log(item);
      ...
      case "trains":
        ...
        return (
          <p
            ...
            style={{
              color:
                train.holidayAddition == "_candyCane" || train.holidayAddition == "_gayFlag"
                  ? "#ffffff"
                  : `#${train.lineTextColor}`,
              background:
                train.holidayAddition == "_candyCane"
                  ? "repeating-linear-gradient(135deg, #94000a, #94000a 10px, #077001 10px, #077001 20px)"
                  : train.holidayAddition == "_gayFlag"
...
export const activateTrainPopup = (feature, map, agencyData) => {
  ...
  const trainElement = (
    <div className="mapBar">
      ...
      {extra && (extra.last_update || extra.last_station) ? (
        <p style={{ marginTop: "-2px", paddingBottom: "4px" }}>
  ...
  const trainPopup = new Popup({ offset: 12, closeButton: true, anchor: "bottom" }).setLngLat(coordinates);

  activatePopup(map, trainElement, trainPopup);
};

export const activateStationPopup = (feature, map, agencyData, singleRouteID) => {
  const station = { ...feature.properties, destinations: JSON.parse(feature.properties.destinations) };
  const coordinates = feature.geometry.coordinates.slice();
  ...
  const predictions = Object.keys(station.destinations).map((destKey, i) => {
    ...
    dest.trains.forEach((train) => {
      if (
        (train.lineCode === singleRouteID || singleRouteID === "all") &&
        train.actualETA >= Date.now() - 1000 * 60 * 5
      )
              ...
              return (
                ...
                >
                  <span>
                    ...
                    {agencyData.tripIDPrefix}
                    {agencyData.runNumberConverter
                      ? agencyData.runNumberConverter(train.runNumber)
                      : train.runNumber}{" "}
                    to <strong>{train.destination ?? destKey}</strong>
                  </span>
  ...
  const staionElement = (
    <div className="mapBar">
      ...
      <p className="mapStationBar" style={{ color: agencyData.textColor, backgroundColor: agencyData.color }}>
        <strong>
          <a
            href={`/${agencyData.agencyID}/stops/${station.stationID}?prev=map`}
            style={{ color: agencyData.textColor, backgroundColor: agencyData.color }}
          >
            View Full Station
          </a>
        </strong>
  ...
  const stationPopup = new Popup({ offset: 8, closeButton: true, anchor: "bottom" }).setLngLat(coordinates);
